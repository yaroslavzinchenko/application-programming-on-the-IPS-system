...
double leftRectanglesMethodOpenMPFixed(double a, double b, double h)
{
	double area = h * (8 / (1 + a * a));

	omp_set_dynamic(0); // Запретить библиотеке openmp менять число потоков во время исполнения.
	omp_set_num_threads(4); // Установить число потоков в 4.
#pragma omp parallel for shared (area) private(a)
	for (; a < b; a = a + h)
	{
		area = area + h * (8 / (1 + a * a));
	}
...
int main()
{
	...
	cout << "Метод левых прямоугольников с использованием OpenMP (fixed):" << endl;
	t1 = chrono::high_resolution_clock::now();
	cout << leftRectanglesMethodOpenMPFixed(a, b, h) << endl;
	t2 = chrono::high_resolution_clock::now();

	chrono::duration <double> durationLeftRectanglesOpenMPFixed = (t2 - t1);
	cout << "Время подсчёта методом левых прямоугольников с использованием OpenMP: " << durationLeftRectanglesOpenMPFixed.count() << " секунд." << endl << endl;
